...
def expensive_tastes
  ...
  execute(<<-SQL)
  -- SELECT
  -- DISTINCT styles.style, SUM(pricess.price)/SUM(countss)
  -- FROM
  -- (
  --   SELECT
  --   .* as pricess, COUNT(tracks.song) as countss
  --   FROM
  --   albums
  ...
  --   album.title
  -- )
  -- JOIN styles ON styles.album = pricess.asin
  -- WHERE
  -- pricess.price IS NOT NULL
  -- GROUP BY
  -- styles.style
  -- -- ORDER BY
  ...
  SELECT
    ...
    JOIN (
      SELECT
        albums.*, -- selecting everything with * and then a count adds the count to the end of the table
      ...
      GROUP BY
        albums.asin
      ) AS album_track_counts ON styles.album = album_track_counts.asin -- you must alias when subquerying
    GROUP BY
      styles.style
    ORDER BY
